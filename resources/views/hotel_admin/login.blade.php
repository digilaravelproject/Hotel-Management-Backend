...
<div class="min-h-screen flex items-center justify-center p-4 bg-slate-950 text-slate-100 relative overflow-hidden">
    ...
    <div class="w-full max-w-md bg-slate-900/90 backdrop-blur-2xl border border-slate-800 rounded-3xl p-8 shadow-2xl shadow-indigo-950/50 space-y-6 relative z-10">
        ...
        <div class="text-center space-y-3">
            ...
            <h1 class="text-2xl font-extrabold text-white tracking-tight">Hotel Admin Portal</h1>
            <p class="text-xs text-slate-400">Access and control your Smart TV network</p>
        </div>
        ...
        <form action="{{ route('hotel.login') }}" method="POST" class="space-y-5">
            ...
            <div class="space-y-1.5">
                <label class="text-xs font-semibold text-slate-300">Email Address (Username)</label>
                <div class="relative">
            ...
            <div class="space-y-1.5">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('hotel.forgot-password') }}" class="text-xs font-medium text-indigo-400 hover:text-indigo-300 transition-colors">Forgot Password?</a>
                </div>
                <div class="relative">
                    <input type="password" id="passwordInput" name="password" required placeholder="••••••••" autocomplete="current-password"
                        class="w-full px-4 py-3 bg-slate-800/80 border border-slate-700/80 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all pr-10">
