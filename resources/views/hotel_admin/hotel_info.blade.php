...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-10 shadow-sm space-y-8">
        <div class="border-b border-slate-100 pb-6 flex items-center space-x-3">
            ...
            <div>
                <h3 class="text-xl font-extrabold text-slate-900 tracking-tight">Hotel Information & Media Gallery</h3>
                <p class="text-xs text-slate-500 font-medium">Update branding, emergency contacts, highlights, and screen images.</p>
            </div>
        </div>
        ...
        <form action="{{ route('hotel.hotel-info') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="description" rows="3" placeholder="Enter a welcome message or overview of your hotel..." 
                        class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:border-indigo-500 focus:bg-white transition-all">{{ old('description', $hotelAdmin->description) }}</textarea>
                </div>
            </div>
                    ...
                    @foreach($currentAmenities as $index => $amenity)
                        <div class="flex items-center space-x-3">
                            <input type="text" name="hotel_amenities[]" value="{{ $amenity }}" placeholder="e.g. 📶 High-Speed Free Wi-Fi" 
                                class="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:border-indigo-500 focus:bg-white transition-all">
            ...
            <div class="space-y-6">
                ...
                <div class="grid sm:grid-cols-2 gap-6">
                    ...
                    <!-- Main Cover Upload -->
                    <div class="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-4">
                        <label class="text-xs font-bold text-slate-800 block">Upload Main Hotel Cover Photo</label>
            ...
            <div class="space-y-6">
                <h4 class="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center space-x-2">
                    <span class="w-6 h-6 rounded-full bg-indigo-600 text-white flex items-center justify-center text-xs">5</span>
                    <span>Hotel Photos / Gallery Images</span>
                </h4>

                <div class="space-y-3">
                    <input type="file" name="hotel_gallery_images[]" accept="image/*" multiple class="w-full text-xs text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-indigo-600 file:text-white hover:file:bg-indigo-500">
                    <p class="text-xs text-slate-400">Multi-select up to 20 gallery images to display on TV app hotel info section.</p>
                </div>
                ...
                @if($hotelAdmin->hotel_gallery_images && count($hotelAdmin->hotel_gallery_images) > 0)
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
                        @foreach($hotelAdmin->hotel_gallery_images as $path)
                            <div class="relative group rounded-2xl overflow-hidden aspect-video border border-slate-200 shadow-sm">
                                <img src="{{ asset($path) }}" alt="Gallery Photo" class="w-full h-full object-cover">
                                <button type="button" onclick="deleteGalleryPhoto('{{ $path }}')" class="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white transition-all">
                                    <span class="w-9 h-9 rounded-full bg-rose-600 flex items-center justify-center text-sm shadow-lg hover:scale-110 transition-transform">
