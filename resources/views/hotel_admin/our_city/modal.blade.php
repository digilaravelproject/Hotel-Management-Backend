...
<script>
    ...
    function triggerEditMode(place) {
        ...
        document.getElementById('highlightsContainer').innerHTML = '';
        highlightsCount = 0;
        if (Array.isArray(place.attractions) && place.attractions.length > 0) {
            place.attractions.forEach(tag => addHighlightInput(tag));
        }
    }
    ...
    function openViewModal(btn) {
        ...
        tagBox.innerHTML = '';
        if (Array.isArray(place.attractions)) {
            place.attractions.forEach(t => {
